...
void pc(const InputType& text, const uint64_t size, const uint64_t levels,
  ContextType& ctx) {
...
void pc_in_external(const InputType& text, const uint64_t size, const uint64_t levels,
  ...
  //~ std::vector<uint64_t> prefix_shift_v(levels);
  //~ std::vector<uint64_t> cur_bit_shift_v(levels);
  //~ uint64_t *prefix_shift = prefix_shift_v.data();
  //~ uint64_t *cur_bit_shift = cur_bit_shift_v.data();
  ...
  //~ for (uint64_t i = 0; i < size; ++i) {
    ...
    //~ ++reader;
    //~ for (uint64_t level = levels - 1; level > 0; --level) {
      //~ const uint64_t pos = borders[level][cur_char >> prefix_shift[level]]++;
      //~ bv[level][pos >> 6] |= (((cur_char >> cur_bit_shift[level]) & 1ULL)
        //~ << (63ULL - (pos & 63ULL)));
    //~ }
  //~ }
