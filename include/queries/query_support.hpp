...
public:
  query_support(wavelet_structure& ws) : ws_(ws) {
    for (size_t i = 0; i < ws_.levels(); ++i) {
      rank_support_.emplace_back(rank_support_type(ws_.bvs()[i]));
      select0_support_.emplace_back(select0_support_type(rank_support_.back()));
      select1_support_.emplace_back(select1_support_type(rank_support_.back()));
    }
  }
